...
<article id="post-<?php the_ID(); ?>" class="<?php hybrid_entry_class(); ?>">
	...
	<?php if ( is_singular() && is_main_query() ) : ?>
		...
		<?php echo apply_atomic_shortcode( 'entry_meta_chat', '<footer class="entry-meta">' . __( '[entry-terms taxonomy="category" before="Posted in "] [entry-terms before="| Tagged "]', 'cakifo' ) . '</footer> <!-- .entry-meta -->' ); ?>
	...
	<?php else : ?>
		...
		<?php echo apply_atomic_shortcode( 'byline_chat', '<footer class="entry-meta">' . __( 'Published on [entry-published] by [entry-author] [entry-permalink before="| "] [entry-comments-link before="| "] [entry-edit-link before=" | "]', 'cakifo' ) . '</footer> <!-- .entry-meta -->' ); ?>
