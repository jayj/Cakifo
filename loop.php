...
<article id="post-<?php the_ID(); ?>" class="<?php hybrid_entry_class(); ?>">
	...
	<?php if ( is_singular() && is_main_query() ) : ?>
		...
		<header class="entry-header">
			...
			<?php echo apply_atomic_shortcode( 'byline_standard', '<div class="byline">' . __( 'Published on [entry-published] by [entry-author] [entry-comments-link before="| "] [entry-edit-link before=" | "]', 'cakifo' ) . '</div>' ); ?>
		</header> <!-- .entry-header -->
		...
		<?php echo apply_atomic_shortcode( 'entry_meta_standard', '<footer class="entry-meta">' . __( '[entry-terms taxonomy="category" before="Posted in "] [entry-terms before="| Tagged "]', 'cakifo' ) . '</footer> <!-- .entry-meta -->' ); ?>
