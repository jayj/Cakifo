...
function hybrid_attachment_id3_keys( $fields, $attachment, $context ) {
	if ( 'display' === $context ) {
		$fields['filesize']         = __( 'File Size', 'cakifo' );
		$fields['mime_type']        = __( 'Mime Type', 'cakifo' );
		$fields['length_formatted'] = __( 'Run Time',  'cakifo' );
	}
	if ( hybrid_attachment_is_audio( $attachment->ID ) ) {
		$fields['genre']        = __( 'Genre',    'cakifo' );
		$fields['year']         = __( 'Year',     'cakifo' );
		$fields['composer']     = __( 'Composer', 'cakifo' );
		$fields['track_number'] = __( 'Track',    'cakifo' );
		if ( 'display' === $context )
			$fields['unsynchronised_lyric'] = __( 'Lyrics', 'cakifo' );
	}
	return $fields;
}
